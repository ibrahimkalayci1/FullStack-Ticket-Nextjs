...
const Form: FC<Props> = ({ isEditMode, editItem }) => {
  return (
    <div className="max-w-[600]">
      <form action={isEditMode ? updateTicketAction : createTicketAction} 
      className="flex flex-col gap-5">
        ...
        <Fieldset label="Kategori" >
            ...
            required defaultValue={editItem?.category} >
     ...
     {TICKET_PRIORITIES.map((priority) => (
        ...
        cursor-pointer hover:text-blue-400" >
         ...
         <Fieldset label={`İlerleme:  `} >
            ...
            min={0} max={100} step={5} defaultValue={editItem?.progress || 0 } />
         </Fieldset>
          
          <Fieldset label="Durum" >
            <select name="status" className="input" 
            required defaultValue={editItem?.status} >
